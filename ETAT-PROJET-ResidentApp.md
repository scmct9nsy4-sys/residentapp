...
- **Cible ATTEINTE (v12, 16/7)** : exécution **nocturne automatique** — Function App `residentapp-soldes-timer`, timer 01:30 UTC (chantier 2b TERMINÉ ; procédure : `soldes-timer/README-SOLDES-TIMER.md`).
...
| **B** | **Azure Function App séparée** (timer trigger), secret en app settings, puis **managed identity** au durcissement (§10.4) | ✅ **RETENUE ET DÉPLOYÉE (v12, 16/7)** — `residentapp-soldes-timer`, Flex Consumption, timer 01:30 UTC |
...
✅ **TERMINÉ (v12, 16/7) — CHANTIER 2b : exécution NOCTURNE de `sp:soldes --auto`.**
Option **B retenue** : Azure **Function App** (`residentapp-soldes-timer`,
**Flex Consumption**, France Central, Node 22) à déclencheur **timer 01:30 UTC**,
dans un dossier `soldes-timer/` du dépôt qui importe `syncAuto()` tel quel
(bundle esbuild autonome, lib inline). Secret Graph en **App Settings**
(managed identity = durcissement §10.4). **Validé en écriture réelle depuis Azure
le 16/7 : `0 créé, 0 mis à jour, 14 799 inchangé` — point fixe global identique à
la CLI** (§7 vérifié en production). ⚠ Le timer ne vit PAS dans `api/` (SWA = HTTP).
📄 **Reconstruction A-à-Z + pièges Flex : `soldes-timer/README-SOLDES-TIMER.md`.**
À retenir : déploiement en **CLI** `func azure functionapp publish
