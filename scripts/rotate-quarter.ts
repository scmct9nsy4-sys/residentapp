 ...
 *    npm run sp:rotate -- T3                 archive puis vide « KB-Cumul T3 »
 ...
 *    - liste introuvable ou vide -> aucune action destructrice.
 *
...
async function main(): Promise<void> {
  ...
  if (items.length === 0) {
    console.log("\nListe déjà vide : rien à archiver ni à vider. Terminé.");
    return;
  }
  ...
  if (exportOnly) {
    console.log("\nMode --export-only : aucune suppression. Terminé.");
    return;
  }
